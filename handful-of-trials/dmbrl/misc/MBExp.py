...
class MBExperiment:
    def __init__(self, params, exp_id, logdir, wandb_name):
        ...
        self.exp_id = exp_id 
        self.logdirg = logdir
        wandb.init(name= wandb_name+"_"+str(self.exp_id), project="pets_analysis_new")
    ...
    def run_experiment(self):
        ...
        for i in range(self.ntrain_iters):
            ...
            step = step + self.task_hor


            reward_sum  = [sample["reward_sum"] for sample in samples[:self.neval]][0]


            wandb.log({"Rewards obtained": reward_sum }, step = step)
            rew_log.append((reward_sum,step))

            print("Rewards obtained:", reward_sum)
        ...
        os.makedirs(os.path.join(self.logdirg,str(self.exp_id)), exist_ok=True)
        pickle.dump(rew_log, open(os.path.join(self.logdirg, str(self.exp_id),"log.p"),"wb"))
